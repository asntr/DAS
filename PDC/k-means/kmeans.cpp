...
vector<size_t> KMeans(const Points& data, size_t K) {
    ...
    while (!converged) {
        ...
        next_centroids.assign(K, Point(dimensions));
        #pragma omp parallel reduction(&& : converged)
        {
            vector<size_t> local_clusters_sizes(K);
            Points local_centroids(K, Point(dimensions));
            #pragma omp for
            for (size_t i = 0; i < data_size; ++i) {
                ...
                for (size_t d = 0; d < dimensions; ++d) {
                    local_centroids[clusters[i]][d] += data[i][d];
                }
            ...
            if (!converged) {
                #pragma omp critical
                {
                    for (size_t i = 0; i < K; ++i) {
                        ...
                        for (size_t d = 0; d < dimensions; ++d) {
                            next_centroids[i][d] += local_centroids[i][d];
                        }
                    }
                }
            }
